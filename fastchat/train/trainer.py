...
class moditrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.periodiclora = periodiclora    
    def _inner_training_loop(
        self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None
    ):
        self.accelerator.free_memory()
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    if self.periodiclora:
                        ...
                        #if True:
                        if self.state.global_step>=update_step:
                            if model.device.index==0:
                                print("global_step>=update_step", self.state.global_step,'/',update_step)
                            ...
                            # if update_step * args.gradient_accumulation_steps >steps_in_epoch:
                            # if update_step > 15:
                            #     print(f'#########剩余步数不足以支持更新{update_step} * {args.gradient_accumulation_steps} >{steps_in_epoch}')
                            ...
                            if self.model_name=='llama':
                                ...
                                for i in range(len(model.base_model.model.model.layers)):
                                    lora_a = model.base_model.model.model.layers[i].self_attn.q_proj.lora_A['default']
                                    lora_b = model.base_model.model.model.layers[i].self_attn.q_proj.lora_B['default']
                                    base = model.base_model.model.model.layers[i].self_attn.q_proj.base_layer
                                    ...
                                    delta_ab = (torch.matmul(lora_a.weight.t(), lora_b.weight.t()))

                                    in_features, r = 4096, 1
                                    with torch.no_grad():
                                        model.base_model.model.model.layers[i].self_attn.q_proj.base_layer.weight.copy_(delta_ab + base.weight)
                                        model.base_model.model.model.layers[i].self_attn.q_proj.lora_A['default'].weight.copy_(nn.Linear(in_features, r, bias=False).weight) 
                                        nn.init.kaiming_uniform_(model.base_model.model.model.layers[i].self_attn.q_proj.lora_A['default'].weight, a=math.sqrt(5))
                                        nn.init.zeros_(model.base_model.model.model.layers[i].self_attn.q_proj.lora_B['default'].weight)
                                        # 过去被使用
                            ...
                            if base.weight.get_device()==0:
                                ...
                                # print("print(self.optimizer)",self.optimizer)
                                ...
                                if hasattr(self.lr_scheduler, "step"):
                                    if self.use_apex:
                                        ...
                                        try:
                                            ...
                                            if base.weight.get_device()==0:
                                                print(f"Caught an AssertionError and ignoring it: {e}")
                                            pass
                                else:
                                    # to handle cases wherein we pass "DummyScheduler" such as when it is specified in DeepSpeed config.
                            ...
                            if base.weight.get_device()==0:
                                # self.optimizer = torch.optim.AdamW(model.parameters(), lr=self.args.learning_rate, betas=(0.9, 0.999))  #min(0.999,1-4/step))
                                # print(f"#################\n({self.optimizer})")
                                print("reset optimizer successfully")
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_save:
            step_savemodel = model
            step_savemodel = step_savemodel.merge_and_unload()
            if model.device.index == 0:
                print('######################保存模型')
                save_id = f"{self.state.global_step}-savemodel"
                savemodel_dir = os.path.join(self.args.output_dir, save_id)
                step_savemodel.save_pretrained(savemodel_dir)
            self.control = self.callback_handler.on_save(self.args, self.state, self.control)

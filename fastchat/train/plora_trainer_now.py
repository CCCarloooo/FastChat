                        ...
                        if self.model_name=='llama':
                            ...
                            for i in range(len(model.base_model.model.model.layers)):
                                for adapter in model.peft_config['default'].target_modules:
                                    if adapter in ['k_proj', 'q_proj', 'o_proj', 'v_proj']:
                                        ...
                                        if args.plora_momentum:
                                            ...
                                            with torch.no_grad():
                                                ...
                                                model.base_model.model.model.layers[i].self_attn.__getattr__(adapter).lora_A['default'].weight.mul_(1-self.plora_momentum_ratio)
                                                model.base_model.model.model.layers[i].self_attn.__getattr__(adapter).lora_B['default'].weight.mul_(1-self.plora_momentum_ratio)
                                        else:
                                    ...
                                    elif adapter in ['up_proj', 'gate_proj', 'down_proj']:
                                        lora_a = model.base_model.model.model.layers[i].mlp.__getattr__(adapter).lora_A['default']
                                        lora_b = model.base_model.model.model.layers[i].mlp.__getattr__(adapter).lora_B['default']
                                        base = model.base_model.model.model.layers[i].mlp.__getattr__(adapter).base_layer
                                        # ipdb.set_trace()
                                        scaling = model.peft_config['default'].lora_alpha / model.peft_config['default'].r
                                        if args.plora_momentum:
                                            delta_ab = lora_b.weight @ lora_a.weight * scaling * self.plora_momentum_ratio
                                            ...
                                            with torch.no_grad():
                                                model.base_model.model.model.layers[i].mlp.__getattr__(adapter).base_layer.weight.copy_(delta_ab + base.weight)
                                                model.base_model.model.model.layers[i].mlp.__getattr__(adapter).lora_A['default'].weight.mul_(1-self.plora_momentum_ratio)
                                                model.base_model.model.model.layers[i].mlp.__getattr__(adapter).lora_B['default'].weight.mul_(1-self.plora_momentum_ratio)
                                            ...
                                            with torch.no_grad():
                                                model.base_model.model.model.layers[i].mlp.__getattr__(adapter).base_layer.weight.copy_(delta_ab + base.weight)
                                                model.base_model.model.model.layers[i].mlp.__getattr__(adapter).lora_A['default'].weight.copy_(nn.Linear(f_in, r, bias=False).weight) 
                                                nn.init.kaiming_uniform_(model.base_model.model.model.layers[i].mlp.__getattr__(adapter).lora_A['default'].weight, a=math.sqrt(5))
                                                nn.init.zeros_(model.base_model.model.model.layers[i].mlp.__getattr__(adapter).lora_B['default'].weight)
